...
public class Logger : ILogger {
  public Task LogClientMessage(LogMessage logMessage) {
    _ = Task.Run(() => {
      if (logMessage.Exception is CommandException commandException) {
        Console.WriteLine($"[Command/{logMessage.Severity}] {commandException.Command.Aliases[0]} failed to execute in {commandException.Context.Channel}.");
  ...
  public void LogException(string fileName, string methodName, Exception ex) {
