...
public class BackupEmbed {
  ...
  private async Task ConstructBackupEmbedAsync() {
    ...
    if (DiscordFactory.GetGuild(_command.GuildId.Value).Roles.Count > 0) {
      AddRoles();
    }
  ...
  private void AddRoles() {
    var orderedRoles = DiscordFactory.GetGuild(_command.GuildId.Value).Roles.OrderByDescending(role => role.Position);
